...
namespace
{
    ...
    AssertAction::AssertAction _defaultHandler(const char *file, int line, const char *function, const char *expression,
                                               int level, const char *message)
    {
        ...
        {
#if (!TARGET_OS_IPHONE && !TARGET_IPHONE_SIMULATOR) && (!defined(__ANDROID__) && !defined(ANDROID)) ||                 \
    defined(PPK_ASSERT_DEFAULT_HANDLER_STDIN)
            for (;;)
            {
                fprintf(stderr, "Press (I)gnore / Ignore (F)orever / Ignore (A)ll / "
                                "(D)ebug / A(b)ort: ");
                fflush(stderr);
...
// The following block (between #if and #endif) has been added by Millian Poquet
// (2016-06-20), inspired by
// https://github.com/gpakosz/Assert/blob/master/test/pempek_assert_test.cpp
...
namespace pempek
{
    namespace assert
    {
        ...
        AssertionException::AssertionException(const char *file, int line, const char *function, const char *expression,
                                               const char *message)
        ...
        namespace implementation
        {
            ...
            void ignoreAllAsserts(bool value) { _ignoreAll = value; }

            bool ignoreAllAsserts() { return _ignoreAll; }
